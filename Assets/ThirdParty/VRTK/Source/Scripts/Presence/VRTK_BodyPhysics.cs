...
namespace VRTK
{
    ...
    public class VRTK_BodyPhysics : VRTK_DestinationMarker
    {
        ...
        {
            var isPlayerObject = VRTK_PlayerObject.IsPlayerObject(checkObject);
 
            //Debug.Log($"CheckValidCollision({checkObject})  isPlayerObject:{isPlayerObject}  currentValidFloorObject:{currentValidFloorObject}   onGround:{onGround}");

            return (!isPlayerObject || !ignoreGrabbedCollisions) && (!onGround || (currentValidFloorObject != null && currentValidFloorObject != checkObject));
        }
