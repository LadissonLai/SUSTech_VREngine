...
#pragma warning restore 0618
        ...
        protected virtual void StopTouchingInteractions()
        {
            ...
            {
                var interactTouch = touchingObject.GetComponentInChildren<VRTK_InteractTouch>();

                if (touchingObject.activeInHierarchy || forceDisabled)
                {
                    //InteractTouch.StopTouching中无法获取已经被取消激活的物体，导致没用调用interactObj的StopTouching方法。
                ...
                {
                    //手动清理
                    StopTouching(interactTouch);
                }
            }

            //Debug.Log(touchingObjects.Count + "|" + name);
        }
